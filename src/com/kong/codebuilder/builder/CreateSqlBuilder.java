...
public class CreateSqlBuilder {
    ...
    private static void generateCreateSql(ClassInfo classInfo) {
        ...
        for (PsiField field : classInfo.getPsiClass().getAllFields()) {
        ...
        String path = MapperBuilderConfigLoader.getInstance().getCreateSqlDirectory().getVirtualFile().getPath();
        String fileName = tableName + ".sql";

        //写文件
        FileUtil.createFile(fileName, path, text);
    }
    ...
    private static String getDefaultField(PsiField field) {
        ClassType fieldClass = ClassType.fromDesc(field.getType().getCanonicalText());
        switch (fieldClass) {
            case STRING:
